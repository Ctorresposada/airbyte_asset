...
| <a name="input_dbt_redshift_user"></a> [dbt\_redshift\_user](#input\_dbt\_redshift\_user) | Redshift database user that dbt Core authenticates as via IAM-brokered credentials. Created with PASSWORD DISABLE so it is reachable only through redshift-serverless:GetCredentials (no static password). Granted USAGE+CREATE+ALL on the gold schema and USAGE+SELECT on the bronze/silver Spectrum schemas. | `string` | `"dbt_service"` | no |
| <a name="input_dbt_task_role_name"></a> [dbt\_task\_role\_name](#input\_dbt\_task\_role\_name) | Name of the IAM role used by the dbt Core ECS task (e.g. 'region-20-dev-dbt-task'). Redshift Serverless GetCredentials maps this IAM role to a database user named IAMR:<role-name> on first connect. Schema grants must target this IAM-derived user — the dbt\_redshift\_user value in the profile is overwritten by the GetCredentials response. | `string` | n/a | yes |
| <a name="input_enable_bastion"></a> [enable\_bastion](#input\_enable\_bastion) | Whether to enable the bastion for Redshift to be reached from outside the VPC | `bool` | `false` | no |
