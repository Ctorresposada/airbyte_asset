...
| <a name="input_glue_crawlers"></a> [glue\_crawlers](#input\_glue\_crawlers) | Map of Glue crawlers to provision. Each entry creates a crawler with its own IAM role, KMS key, and security configuration. | <pre>map(object({<br/>    s3_bucket_key = string<br/>    s3_prefix     = string<br/>    database_key  = string<br/>    table_prefix  = string<br/>    schedule      = string<br/>  }))</pre> | `{}` | no |
