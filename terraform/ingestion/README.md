...
| [aws_iam_role_policy.lakeformation_get_data_access](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
...
| <a name="input_lakeformation_analyst_role_names"></a> [lakeformation\_analyst\_role\_names](#input\_lakeformation\_analyst\_role\_names) | Names of IAM roles for the Analyst SSO permission set (read-only). Granted lakeformation:GetDataAccess via an inline IAM policy so the Analyst team can query Glue tables via Athena. | `list(string)` | `[]` | no |
| <a name="input_lakeformation_auditor_role_names"></a> [lakeformation\_auditor\_role\_names](#input\_lakeformation\_auditor\_role\_names) | Names of IAM roles for the Auditor SSO permission set (read-only). Granted lakeformation:GetDataAccess via an inline IAM policy so auditors can query Glue tables via Athena. | `list(string)` | `[]` | no |
| <a name="input_lakeformation_de_role_names"></a> [lakeformation\_de\_role\_names](#input\_lakeformation\_de\_role\_names) | Names of IAM roles for the Data Engineer SSO permission set. Granted lakeformation:GetDataAccess via an inline IAM policy (SSO reserved roles cannot be LF admins). In a future state where IAMAllowedPrincipals defaults are removed, these roles will also receive explicit LF CREATE\_TABLE + SELECT grants. | `list(string)` | `[]` | no |
